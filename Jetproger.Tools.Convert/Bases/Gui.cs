using System;
using System.Drawing;
using Jetproger.Tools.Convert.Converts;
using Jetproger.Tools.Convert.Factories;

namespace Jetproger.Tools.Convert.Bases
{
    public static class GuiExtensions
    {
        public static Icon ImageAsIcon(this f.IGuiExpander exp, Image image, int width, int height)
        {
            try
            {
                var bmp = new Bitmap(image, new Size(width, height));
                bmp.MakeTransparent(Color.Magenta);
                var hIcon = bmp.GetHicon();
                return Icon.FromHandle(hIcon);
            }
            catch
            {
                return DefaultIcon(exp);
            }
        }

        public static Image DefaultImage(this f.IGuiExpander exp)
        {
            return t<Image>.one(() => f.str.to<Image>(_DefaultImage(exp)));
        }

        public static Icon DefaultIcon(this f.IGuiExpander exp)
        {
            return t<Icon>.one(() => f.str.to<Icon>(_DefaultIcon(exp)));
        }

        public static string _DefaultIcon(this f.IGuiExpander exp)
        {
            return @"";
        }

        public static string _DefaultImage(this f.IGuiExpander exp)
        {
            return @"";
        }
    }
}